...
@click.option('-o', '--output', show_default=True, default='targets.txt',
              help='Path to output file with parsed data. IP:PORT:ORG:HOSTNAMES format, one entry per line')
...
def cli(shodan_json_file, output, verbose, log_every):
    ...
    with open(shodan_json_file) as jsonFile, open(output, 'w') as fileToWrite:
        ...
        for line in jsonFile:
            ...
            org = str(jsonObject.get('org'))
            hostnames = str(jsonObject.get('hostnames'))

            try:
                if ipaddress.ip_address(ip):
                    writableOutput = f'{ip}:{port}:{org}:{hostnames}'
                    fileToWrite.write("%s\n" % writableOutput)
                    correctIpCounter += 1
            except ValueError:
